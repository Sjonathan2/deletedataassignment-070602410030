...
<form action="{{ route('insert_product') }}" method="POST" enctype="multipart/form-data" class="row g-3">
    @csrf
    <div class="col-md-6">
    ...
    <div class="col-md-6">
        ...
        <select id="product_category" name="product_category" class="form-select @error('product_category') is-invalid @enderror">
            ...
            <option value="{{ $pc->id }}" {{ old('product_category') == $pc->id ? 'selected' : '' }}>{{ $pc->name }}</option>
            @endforeach
        </select>
